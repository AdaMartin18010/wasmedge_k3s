...
graph TD
    subgraph 省却本体层
        SAVE[省却 Savelett]
        SAVE -->|类型| SAVE_CODE[代码省却 Codelett]
        SAVE -->|类型| SAVE_COMP[组件省却 Comlett]
        SAVE -->|类型| SAVE_COG[认知省却 Coglett]
    end

    subgraph 代码省却 Codelett
        SAVE_CODE -->|实现| AUTO_INSTRUMENT[自动插桩 AutoProbe]
        SAVE_CODE -->|实现| ZERO埋点[零埋点 ZeroHook]
        SAVE_CODE -->|实现| CONTEXT_PROPAGATION[自动上下文传递 ContextFlow]
    ...
    subgraph 组件省却 Comlett
        SAVE_COMP -->|消除| SIDECARLESS[Sidecarless架构]
        SAVE_COMP -->|消除| AGENTLESS[Agentless采集]
        SAVE_COMP -->|合并| UNIFIED_PIPELINE[统一处理管道 UnifiedPipe]

        SIDECARLESS -->|替代| DAEMONSET[DaemonSet单例]
        AGENTLESS -->|替代| KERNEL_PROBE[内核级探针]
        UNIFIED_PIPELINE -->|替代| OTLP_ARROW[OTLP Arrow列式编码]
    end

    subgraph 认知省却 Coglett
        SAVE_COG -->|降低| LEARNING_CURVE[学习曲线陡峭度]
        SAVE_COG -->|简化| MENTAL_MODEL[心智模型复杂度]
        SAVE_COG -->|聚焦| BUSS_LOGIC[业务逻辑专注度]

        LEARNING_CURVE -->|从| MULTI_STACK[多技术栈: Prom/Jaeger/ES]
        LEARNING_CURVE -->|到| ONE_MODEL[单一OTLP语义模型]

        MENTAL_MODEL -->|从| OBSERVABILITY_FIRST[观测优先设计]
        MENTAL_MODEL -->|到| BUSSINESS_FIRST[业务优先设计]
    end

    subgraph 价值产出层
        SAVE_CODE -->|产出| DEV_VELOCITY[开发速度↑3x]
        SAVE_COMP -->|产出| RESOURCE_SAVE[资源节省↑90%]
        SAVE_COG -->|产出| MTTR缩减[MTTR↓95%]

        DEV_VELOCITY -->|量化| PERSON_WEEK[11.15人周/2周 sprint]
        RESOURCE_SAVE -->|量化| MEMORY_SAVE[9.8GB内存/100Pod]
        MTTR缩减 -->|量化| TIME_SAVE[从4小时→5分钟]
    end
    ...
    subgraph 技术支撑层
        ...
        eBPF -->|提供| KERNEL_SANDBOX[内核沙盒]
        eBPF -->|提供| ZERO_OVERHEAD[零运行时开销]

        OTLP -->|提供| STANDARD_SEMANTICS[标准化语义]
        OTLP -->|提供| COLUMNAR_ENCODING[列式编码]
...
```mermaid
gantt
    title eBPF+OTLP技术栈演进时间线
    dateFormat YYYY-MM-DD
    section 传统架构阶段
    SDK埋点           :done, 2020-01-01, 2022-06-30
    Sidecar激增       :done, 2021-03-01, 2023-12-31
    维护成本危机      :done, 2022-01-01, 2024-06-30

    section 过渡阶段
    BCC快速原型       :active, 2022-07-01, 2023-12-31
    CO-RE生产试点     :active, 2023-01-01, 2024-06-30
    OTLP统一接入      :active, 2023-07-01, 2024-12-31

    section 未来架构
    Sidecarless普及   :milestone, 2024-07-01, 0d
    Arrow列式编码GA   :milestone, 2024-10-01, 0d
    内核态自治        :future, 2025-01-01, 2026-12-31
    完全可观测性      :future, 2025-07-01, 2027-12-31

    section 价值实现
    代码量↓95%        :milestone, 2024-03-01, 0d
    组件数↓69%        :milestone, 2024-09-01, 0d
    MTTR↓95%          :milestone, 2024-12-01, 0d
```
...
graph LR
    subgraph 技术层省却
        T1[代码行↓95%<br/>1850→80行]
        T2[组件数↓69%<br/>13→4个]
        T3[维护工时↓97%<br/>4000h→10h]
        T4[资源开销↓90%<br/>18GB→0.2GB]
    end

    subgraph 工程层收益
        E1[开发速度↑3x]
        E2[故障定位↓95%<br/>4h→5min]
        E3[部署密度↑5x<br/>同资源多跑5倍Pod]
        E4[变更失败率↓60%<br/>代码量少]
    end
...
```mermaid
timeline
    title eBPF+SDK混合架构演进 (2024-2026)

    section 2024 Q3
        混合架构1.0 : 70% eBPF广度 + 30% SDK深度
        标志 : CO-RE生产落地
        SDK省却率 : 50% (仅保留业务埋点)

    section 2024 Q4
        混合架构2.0 : 80% eBPF + 20% SDK
        标志 : Arrow列式编码GA
        新增能力 : eBPF自动W3C Trace Context注入
        SDK省却率 : 60% (SDK专注纯业务)

    section 2025 Q1
        混合架构3.0 : 85% eBPF + 15% SDK
        标志 : Python/Node.js符号解析完善
        新增能力 : eBPF自动关联Kafka消息
        SDK省却率 : 70% (仅异步场景)

    section 2025 Q2
        混合架构4.0 : 90% eBPF + 10% SDK
        标志 : 内核态ML推理框架
        新增能力 : 预测性埋点（AI决定何时需SDK补充）
        SDK省却率 : 80% (按需埋点)

    section 2025 Q4
        自治架构 : 95% eBPF + 5% SDK
        标志 : 业务语义自动推断（研究阶段）
        新增能力 : NLP解析日志→自动提取业务实体
        SDK省却率 : 90% (仅极端场景)
```
...
graph TD
    ...
    Q1 -->|系统资源、网络、安全| A1[纯eBPF]
    Q1 -->|业务逻辑、事务、异步| A2[需SDK]

    A1 --> Q2[内核版本>=4.14?]
    Q2 -->|是| A3[使用CO-RE libbpf]
    Q2 -->|否| A4[使用BCC（过渡期）]
    ...
    A3 --> Q4[需要关联业务标签?]
    Q4 -->|是| A7[eBPF + SDK补充业务标签]
    Q4 -->|否| A8[纯eBPF，无需SDK]

    A5 --> Q5[需要深度追踪?]
    Q5 -->|是| A9[SDK为主 + eBPF自动补充系统Span]
    Q5 -->|否| A10[SDK仅Metrics/Logs]
...
mindmap
  root((TraceContext传播))
    ...
    传播载体层
      HTTP Header
        自动注入: eBPF在tcp_sendmsg拦截HTTP请求
        手动注入: SDK在http.Client添加Header
        提取点: eBPF在tcp_recvmsg解析Header
      gRPC Metadata
        自动注入: gRPC-Go的stats.Handler
        手动注入: interceptors
        提取点: eBPF解析HTTP/2帧
      消息队列
        Kafka Record Header
          手动注入: Producer添加traceparent
          手动提取: Consumer提取traceparent
          eBPF局限: 无法解析二进制消息体
        RabbitMQ Properties
          手动注入: MessageProperties
          手动提取: MessageListener

    跨进程边界
      同进程多线程
        In-Context传递: thread-local Context
        eBPF视角: 看到pthread_create但无TraceContext
      跨进程同节点
        Unix Domain Socket
...
```mermaid
mindmap
  root((未闭环问题 Root Causes))

    协议层未闭环
      W3C TraceContext无eBPF字段
        现状: traceparent在HTTP头，eBPF需解析skb
        问题: HTTP/2二进制帧解析复杂，性能开销高
        方向: TraceContext v2增加内核可见字段
      缺乏内核态Context标准
        现状: thread-local是用户态概念
        问题: eBPF无法访问Go/Python的Context
        方向: 运行时暴露trace_id到task_struct
    ...
    工具链未闭环
      eBPF程序生命周期管理
        ...
        配置: ConfigMap + Map更新
        卸载: 需手动清理
        问题: 没有Kubernetes-native的operator
      Map数据竞争
        并发: 多个eBPF程序写同一个Map
        一致性: 无事务支持
        方向: 引入bpf_spin_lock
    ...
    生态成熟度未闭环
      动态语言支持
        Python: 需要pyroscope+USDT
        Node.js: 需要llnode+v8 debug
        问题: 符号解析成功率<60%
        方向: 运行时原生支持BTF
      中间件协议
        Kafka: 二进制协议，Header解析困难
        Redis: RESP协议，无标准trace字段
        问题: eBPF无法自动注入TraceContext
        方向: 中间件原生支持W3C标准
