...
- [云原生技术栈认知视图：Docker → K8s/K3s → WasmEdge → OPA](#云原生技术栈认知视图docker--k8sk3s--wasmedge--opa)
  - [目录](#目录)
  - [⚡ 快速导航](#-快速导航)
    - [🎯 按角色快速跳转](#-按角色快速跳转)
    - [📚 按主题快速跳转](#-按主题快速跳转)
...
> **提示**：本文档共 992 行，建议根据需求选择阅读路径。

### 🎯 按角色快速跳转

- **👨‍💻 开发
  者**：[3.3 落地路径：10 分钟跑通"K3s + WasmEdge"](#33-落地路径10-分钟跑通k3s--wasmedge)
  →
  [4.3 落地路径：10 分钟跑通"K3s + WasmEdge + OPA"准入控制](#43-落地路径10-分钟跑通k3s--wasmedge--opa准入控制)
  → [8.1 2025 年一键安装命令](#81-2025-年一键安装命令)
- **🏗️ 架构
  师**：[2.1 理念层：从"货运集装箱"到"声明式宇宙"](#21-理念层从货运集装箱到声明式宇宙)
  → [7. 矩阵视角：云原生矩阵力学](#7-矩阵视角云原生矩阵力学) →
  [6. 技术-场景矩阵（9×6 全景）](#6-技术-场景矩阵96-全景)
- **🔧 运维工程师**：[2.3 技术层：逐层论证](#23-技术层逐层论证) →
  [3.4 性能对比](#34-性能对比同硬件-4c8g100-并发冷启动) →
  [4.4 性能实测](#44-性能实测4c8g1000-qps-准入请求)
- **🔬 研究人员**：[7. 矩阵视角：云原生矩阵力学](#7-矩阵视角云原生矩阵力学) →
  [2.2 知识结构：三维思维导图](#22-知识结构三维思维导图) →
  [6.3 知识图谱（RDF 三元组速记）](#63-知识图谱rdf-三元组速记)
...
- **技术演
  进**：[2. Docker → K8s → K3s：容器技术栈演进](#2-docker--k8s--k3s容器技术栈演进)
  → [2.4 时间轴：技术演进历程](#24-时间轴技术演进历程)
- **WasmEdge 集
  成**：[3. WasmEdge 集成：WebAssembly 作为一等公民](#3-wasmedge-集成webassembly-作为一等公民)
  → [3.3 落地路径](#33-落地路径10-分钟跑通k3s--wasmedge)
- **OPA 集
  成**：[4. OPA 集成：策略即代码的轻量级落地](#4-opa-集成策略即代码的轻量级落地)
  → [4.3 落地路径](#43-落地路径10-分钟跑通k3s--wasmedge--opa准入控制)
- **技术趋势**：[5. 2025 年技术趋势](#5-2025-年技术趋势)
- **技术矩阵**：[6. 技术-场景矩阵（9×6 全景）](#6-技术-场景矩阵96-全景)
- **矩阵力学**：[7. 矩阵视角：云原生矩阵力学](#7-矩阵视角云原生矩阵力学)

### 🚀 快速开始（5 分钟）

1. 阅读 [1. 概述](#1-概述) 了解核心演进路径
2. 查看 [8.1 2025 年一键安装命令](#81-2025-年一键安装命令) 快速上手
3. 参考 [3.3 落地路径](#33-落地路径10-分钟跑通k3s--wasmedge) 实践 WasmEdge 集成
