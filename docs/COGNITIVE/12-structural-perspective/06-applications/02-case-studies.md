...
- **[结构主义设计指南](01-design-guidelines.md)** - 如何运用结构主义思想进行系统设计
  - **内容**：设计原则（结构决定性质、结构平衡、结构复合）、设计流程（需求分析、结构选型、结构验证、结构优化）、结构选型矩阵、结构验证检查清单
  - **适用场景**：系统设计和架构设计

#### 结构失衡与故障模式

- **[结构失衡与故障模式](../05-tech-stack-analysis/03-failure-modes.md)** - Meltdown、Spectre、Docker rm -rf / 等故障的结构分析
  - **内容**：故障的结构分析、各故障详细分析、故障预防策略
  - **适用场景**：故障分析和预防

#### 结构选型原则

- **[结构选型原则](../04-virtualization-analysis/03-selection-principles.md)** - 结构主义选型格言和原则
  - **内容**：结构主义选型格言（"你要多假的机器？"）、选型/迁移口诀（"向下买计算，向上买控制，向边买信息"）、选型决策矩阵
  - **适用场景**：技术选型和架构决策
