...
- [04.6 性能基准要求](#046-性能基准要求)
  - [04.6.1 数据来源要求](#0461-数据来源要求)
  - [04.6.2 版本和日期要求](#0462-版本和日期要求)
  - [04.6.3 测试环境要求](#0463-测试环境要求)
- [04.7 性能优化建议](#047-性能优化建议)
  - [04.7.1 启动性能优化](#0471-启动性能优化)
  - [04.7.2 资源占用优化](#0472-资源占用优化)
  - [04.7.3 部署密度优化](#0473-部署密度优化)
- [04.8 参考](#048-参考)
...
| ------------------- | ------------------ | --------------- | ------------- | ------------------------ |
| **启动时间**        | 1.2s               | 6ms             | **200× 更快** | WasmEdge 0.04.0, 2025-01 |
| **启动延迟（p99）** | 2.5s               | 12ms            | **208× 更快** | WasmEdge 0.04.0, 2025-01 |
...
| 指标              | 传统容器（alpine） | WasmEdge + crun | 提升倍数      | 来源/版本                |
| ----------------- | ------------------ | --------------- | ------------- | ------------------------ |
| **镜像体积**      | 13 MB              | 0.9 MB          | **14× 更小**  | WasmEdge 0.04.0, 2025-01 |
| **内存基线**      | 18 MB              | 2.1 MB          | **8.5× 更小** | WasmEdge 0.04.0, 2025-01 |
| **内存峰值**      | 35 MB              | 5 MB            | **7× 更小**   | WasmEdge 0.04.0, 2025-01 |
| **运行 CPU 基线** | 2%                 | 0.5%            | **4× 更低**   | WasmEdge 0.04.0, 2025-01 |
...
| 指标                | 传统容器（alpine） | WasmEdge + crun | 提升倍数     | 来源/版本                |
| ------------------- | ------------------ | --------------- | ------------ | ------------------------ |
| **单节点密度**      | 300 Pod            | 3000 Pod        | **10× 更高** | WasmEdge 0.04.0, 2025-01 |
| **冷启动 CPU 尖峰** | 80%                | 3%              | **26× 更低** | WasmEdge 0.04.0, 2025-01 |
...
| ------------ | ----------- | -------- | ------------ | ------------------------------------ |
| **启动时间** | 100ms       | 6ms      | **16× 更快** | OPA 0.58.0, WasmEdge 0.04.0, 2025-01 |
| **内存占用** | 50 MB       | 2 MB     | **25× 更小** | OPA 0.58.0, WasmEdge 0.04.0, 2025-01 |
| **评估延迟** | 10ms        | 1ms      | **10× 更快** | OPA 0.58.0, WasmEdge 0.04.0, 2025-01 |
...
- ✅ **技术版本**：必须注明技术版本（如 WasmEdge 0.04.0）
