...
| 现象                  | 问题类别     | 解决方案章节              | 优先级 |
| --------------------- | ------------ | ------------------------- | ------ |
| **Pod 无法启动**      | 运行时问题   | [3.2](#0632-运行时问题)   | P1     |
| **kubectl logs 为空** | 运行时问题   | [3.2](#0632-运行时问题)   | P2     |
| **镜像拉取失败**      | 运行时问题   | [3.2](#0632-运行时问题)   | P1     |
| **网络不通**          | 网络存储问题 | [3.3](#0633-网络存储问题) | P1     |
| **sqlite 文件损坏**   | 网络存储问题 | [3.3](#0633-网络存储问题) | P0     |
| **Webhook 超时**      | 策略配置问题 | [3.4](#0634-策略配置问题) | P1     |
| **启动时间过长**      | 性能优化问题 | [3.5](#0635-性能优化问题) | P3     |
| **HPA 不触发**        | 性能优化问题 | [3.5](#0635-性能优化问题) | P2     |

### 8.2 按部署场景检索

| 场景           | 常见问题                      | 解决方案章节                 | 优先级 |
| -------------- | ----------------------------- | ---------------------------- | ------ |
| **边缘计算**   | 离线安装、资源受限、网络分区  | [4.1](#0641-边缘计算场景)    | P1     |
| **Serverless** | 冷启动慢、密度低、扩缩容问题  | [4.2](#0642-serverless-场景) | P1     |
| **AI 推理**    | 推理延迟、GPU 问题、模型体积  | [4.3](#0643-ai-推理场景)     | P1     |
| **混合云**     | 管理问题、网络同步、策略统一  | [4.4](#0644-混合云场景)      | P1     |
| **微服务架构** | Sidecar、mTLS、路由、可观测性 | [4.5](#0645-微服务架构场景)  | P1     |

### 8.3 按技术组件检索

| 组件         | 常见问题                              | 解决方案章节                                        | 优先级 |
| ------------ | ------------------------------------- | --------------------------------------------------- | ------ |
| **K3s**      | 安装失败、节点无法加入、存储问题      | [3.1](#0631-安装部署问题) [3.3](#0633-网络存储问题) | P1     |
| **WasmEdge** | 日志为空、镜像拉取失败、out of bounds | [3.2](#0632-运行时问题)                             | P1-P2  |
| **OPA-Wasm** | Webhook 超时、策略更新未生效          | [3.4](#0634-策略配置问题)                           | P1-P2  |
| **crun**     | 版本过低、Wasm 识别失败               | [3.1](#0631-安装部署问题)                           | P2     |
