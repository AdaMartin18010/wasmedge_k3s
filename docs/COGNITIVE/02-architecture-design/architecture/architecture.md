...
```mermaid
C4Container
    title Docker/Kubernetes/K3s 架构容器视图

    System_Boundary(docker, "Docker 架构") {
        Container(docker_cli, "docker-cli", "CLI工具", "用户接口")
        Container(dockerd, "dockerd", "守护进程", "镜像和容器管理")
        Container(containerd, "containerd", "运行时接口", "容器生命周期")
        Container(runc, "runc", "OCI运行时", "创建容器进程")
    }

    System_Boundary(k8s, "Kubernetes 架构") {
        Container(api_server, "API Server", "统一入口", "API网关")
        Container(etcd, "etcd", "状态存储", "分布式键值存储")
        Container(controller, "Controller", "状态管理", "控制循环")
        Container(scheduler, "Scheduler", "调度决策", "Pod调度")
        Container(kubelet, "kubelet", "节点代理", "Pod生命周期")
    }

    System_Boundary(k3s, "K3s 架构") {
        Container(k3s_binary, "k3s二进制", "单二进制", "嵌入式组件")
        Container(sqlite, "sqlite", "本地存储", "单文件数据库")
    }

    Rel(docker_cli, dockerd, "调用")
    Rel(dockerd, containerd, "调用")
    Rel(containerd, runc, "调用")
    Rel(api_server, etcd, "读写")
    Rel(controller, api_server, "监听")
    Rel(scheduler, api_server, "监听")
    Rel(kubelet, api_server, "监听")
    Rel(k3s_binary, sqlite, "读写")
```
