...
- [12.4.1 结构主义三棱镜分析](#1241-结构主义三棱镜分析)
  - [📑 目录](#-目录)
  ...
  - [12.4.1.3 结构三角形](#12413-结构三角形)
    - [可视化表示](#可视化表示)
    - [位置分析](#位置分析)
      - [虚拟化：几乎垂直拉高"计算结构"轴](#虚拟化几乎垂直拉高计算结构轴)
      - [容器化：落在**计算-控制边缘**](#容器化落在计算-控制边缘)
      - [沙盒化：紧贴**信息结构**角](#沙盒化紧贴信息结构角)
...
三条技术在结构三角形中的位置反映了它们在计算、控制、信息三轴上的权重分配。
...
- **硬件级状态机完整克隆**：通过虚拟化硬件（CPU、内存、I/O）实现完整的计算环境
- **控制与信息可伪装**：可以伪造硬件信息（ACPI 表、PCI 拓扑），但首要保证计算语
  义一致
- **全局序保证**：VM-Exit/Entry 形成全局同步点，确保计算的一致性和可预测性

**结构洞察**：虚拟化的核心是保证计算结构的完整性，即使控制结构和信息结构可以被伪
装，计算语义必须保持一致。
...
- **仍需 syscall 级重定向**（控制）：通过 namespace 和 cgroup 实现进程隔离和资源
  控制
- **但不需要指令级仿真**（计算轻一半）：进程继续发原生指令，只有 syscall 被重定
  向
- **信息结构优化**：镜像分层 = 信息拓扑的"近似"（只读层+写层=开集）

**结构洞察**：容器化在计算结构和控制结构之间取得平衡，通过优化信息结构（镜像分层
、存储）实现了快速启动和资源效率。
...
- **计算零修改**：用户态指令零修改，保持原有的计算语义
- **控制只加过滤点**：通过 seccomp-bpf、pledge/unveil 等机制过滤危险操作
- **核心在最小可见性/最小权限**：信息拓扑的"开覆盖"最小化，只暴露必要的资源

**结构洞察**：沙盒化的核心是信息结构的优化，通过最小化信息可见性和权限，实现安全
隔离，而不需要改变计算结构和控制结构。
...
- **[复合技术分析](02-composite-technologies.md)** -
  ...
  - **内容**：复合技术的结构分析、各技术详细分析
    （KVM+QEMU、gVisor、Firecracker、Docker+seccomp、WASM 沙盒）
