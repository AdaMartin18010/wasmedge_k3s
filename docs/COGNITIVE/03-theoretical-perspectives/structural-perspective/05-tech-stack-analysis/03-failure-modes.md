...
- [12.5.3 结构失衡与故障模式](#1253-结构失衡与故障模式)
  - [📑 目录](#-目录)
  - [12.5.3.1 故障的结构分析](#12531-故障的结构分析)
    - [故障类型与结构失衡](#故障类型与结构失衡)
  - [12.5.3.2 各故障详细分析](#12532-各故障详细分析)
    - [1. Meltdown：信息结构崩溃](#1-meltdown信息结构崩溃)
    - [2. Spectre：控制结构被利用](#2-spectre控制结构被利用)
    - [3. Docker rm -rf /：计算结构未隔离](#3-docker-rm--rf-计算结构未隔离)
    - [4. Java STW 卡顿：控制结构单点](#4-java-stw-卡顿控制结构单点)
    - [5. 浮点 nondeterminism：计算/信息失衡](#5-浮点-nondeterminism计算信息失衡)
...
- **[结构流分析](02-structure-flow.md)** - 计算结构、控制结构、信息结构在调用链
  中的传递
  - **内容**：调用链的结构流、计算结构下传、控制结构转折、信息结构短路
  - **适用场景**：理解结构在调用链中的传递机制
