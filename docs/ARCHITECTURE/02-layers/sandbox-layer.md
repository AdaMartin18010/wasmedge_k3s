...
- [4. 技术实现](#4-技术实现)
  ...
  - [2. gVisor (用户态内核)](#2-gvisor-用户态内核)
  - [3. Firecracker (轻量级 MicroVM)](#3-firecracker-轻量级-microvm)
  - [4. WasmEdge (WebAssembly 运行时)](#4-wasmedge-webassembly-运行时)
- [5. 对比矩阵](#5-对比矩阵)
- [6. 安全模型](#6-安全模型)
  - [1. 能力闭包（Capability Closure）](#1-能力闭包capability-closure)
  - [2. 最小权限原则](#2-最小权限原则)
  - [3. 可编程策略](#3-可编程策略)
- [7. 与其他层的关系](#7-与其他层的关系)
  - [与容器运行时层](#与容器运行时层)
  ...
  - [与 OPA 策略层](#与-opa-策略层)
- [8. 演进路径](#8-演进路径)
  - [第一阶段：基础隔离（2010-2015）](#第一阶段基础隔离2010-2015)
  - [第二阶段：系统调用过滤（2015-2020）](#第二阶段系统调用过滤2015-2020)
  - [第三阶段：用户态内核（2020-2025）](#第三阶段用户态内核2020-2025)
  - [第四阶段：WebAssembly 沙盒（2025-）](#第四阶段webassembly-沙盒2025-)
- [9. 最佳实践](#9-最佳实践)
  - [1. 最小权限原则](#1-最小权限原则)
  - [2. 分层防御](#2-分层防御)
  - [3. 策略即代码](#3-策略即代码)
...
### 3. Firecracker (轻量级 MicroVM)

- **内存占用**：< 5 MB
- **启动时间**：< 125 ms
- **适用场景**：Serverless、边缘计算

### 4. WasmEdge (WebAssembly 运行时)
...
### 第二阶段：系统调用过滤（2015-2020）

- **技术**：seccomp、AppArmor
- **特点**：细粒度权限控制

### 第三阶段：用户态内核（2020-2025）

- **技术**：gVisor、Firecracker
- **特点**：完整内核隔离

### 第四阶段：WebAssembly 沙盒（2025-）
