...
- [6. 安全模型](#6-安全模型)
  - [1. 身份驱动拓扑](#1-身份驱动拓扑)
  - [2. mTLS 自动轮转](#2-mtls-自动轮转)
  - [3. 零信任网络](#3-零信任网络)
- [7. 与 Network Service Mesh 的关系](#7-与-network-service-mesh-的关系)
  - [Service Mesh 作为 Network Service](#service-mesh-作为-network-service)
- [8. 节点聚合：从物理地址到身份驱动拓扑](#8-节点聚合从物理地址到身份驱动拓扑)
  - [传统模型 vs Service Mesh 模型](#传统模型-vs-service-mesh-模型)
  - [聚合逻辑成为声明式配置](#聚合逻辑成为声明式配置)
- [9. 服务组合：从跨服务流到可编排的本地函数](#9-服务组合从跨服务流到可编排的本地函数)
  - [Envoy Filter Chain](#envoy-filter-chain)
  - [组合语义上升到架构描述层](#组合语义上升到架构描述层)
- [10. 架构设计范式重塑](#10-架构设计范式重塑)
  - [1. "先定接口，再定部署" → "先定流量，再定接口"](#1-先定接口再定部署--先定流量再定接口)
  - [2. 非功能性从"后期治理"变为"设计期可组合元素"](#2-非功能性从后期治理变为设计期可组合元素)
- [11. 最佳实践](#11-最佳实践)
  - [1. 渐进式采用](#1-渐进式采用)
  - [2. 统一配置管理](#2-统一配置管理)
  - [3. 可观测性优先](#3-可观测性优先)
