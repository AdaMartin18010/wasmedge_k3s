...
- [OPA 策略治理：从策略即代码到可证明安全](#opa-策略治理从策略即代码到可证明安全)
  - [📑 目录](#-目录)
  - [1 核心命题](#1-核心命题)
    - [1.1 OPA 在 ℳ 模型中的定位](#11-opa-在-ℳ-模型中的定位)
    - [1.2 形式化描述](#12-形式化描述)
  ...
  - [5 第二次归纳映射——把"能力闭包"下沉到沙盒](#5-第二次归纳映射把能力闭包下沉到沙盒)
    - [5.1 场景：gVisor + OPA](#51-场景gvisor--opa)
    - [5.2 形式化描述](#52-形式化描述)
    - [5.3 实证分析](#53-实证分析)
  - [6 第三次归纳映射——把"服务间权限"组合化](#6-第三次归纳映射把服务间权限组合化)
    - [6.1 场景：Service Mesh + OPA](#61-场景service-mesh--opa)
    - [6.2 Rego 示例](#62-rego-示例)
    - [6.3 归纳收益](#63-归纳收益)
  - [7 OPA 体系结构（范畴论视角）](#7-opa-体系结构范畴论视角)
    - [7.1 架构图](#71-架构图)
    - [7.2 核心组件](#72-核心组件)
  - [8 OPA 在层次模型中的定位](#8-opa-在层次模型中的定位)
    - [8.1 定位矩阵](#81-定位矩阵)
  - [9 OPA 与 Service Mesh / NSM 的组合](#9-opa-与-service-mesh--nsm-的组合)
    - [9.1 侧车-PDP 组合（Istio + OPA）](#91-侧车-pdp-组合istio--opa)
    - [9.2 NSM 级别策略](#92-nsm-级别策略)
    - [9.3 Admission \& Deployment](#93-admission--deployment)
  - [10 OPA 在 CI/CD 与 GitOps 里的角色](#10-opa-在-cicd-与-gitops-里的角色)
    - [10.1 流程矩阵](#101-流程矩阵)
  - [11 封闭证明——OPA 让 ℳ 获得"可证明安全"](#11-封闭证明opa-让-ℳ-获得可证明安全)
    - [11.1 待证命题 P(n)](#111-待证命题-pn)
    - [11.2 基础步](#112-基础步)
    - [11.3 归纳步](#113-归纳步)
    - [11.4 结论](#114-结论)
  - [12 总结](#12-总结)
