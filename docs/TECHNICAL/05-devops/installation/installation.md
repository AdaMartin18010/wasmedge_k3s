...
- [10. 安装与部署：K3s + WasmEdge + OPA 完整指南](#10-安装与部署k3s--wasmedge--opa-完整指南)
  - [📑 目录](#-目录)
  - [10.1 文档定位](#101-文档定位)
  - [10.2 前置要求](#102-前置要求)
...
- **Kubernetes**：1.30（已发布，但支持期已于2025-07-15结束，建议升级至1.31或1.32）
- **K3s**：1.30.4+k3s1（内置 WasmEdge 驱动，`--wasm` flag，待验证）
- **WasmEdge**：0.14.0（内置 Llama2/7B 插件，待验证）
- **Gatekeeper**：v3.15.x（支持 Wasm 引擎，待验证）
- **一键安装**：所有命令已验证（2025-11-06）

**版本状态说明**：

- ⚠️ **Kubernetes 1.30**：支持期已于2025年7月15日结束，建议在生产环境使用 Kubernetes 1.31 或 1.32
- ⚠️ **版本验证状态**：部分版本信息待进一步验证，详见 [版本验证结果记录](../../../../VERSION-VERIFICATION-RESULTS.md)
