...
- [L-2 半虚拟化层（Guest 内核配合）](#l-2-半虚拟化层guest-内核配合)
  - [📑 目录](#-目录)
...
> **📌 重要说明**：L-2 半虚拟化层的核心特征是**需要修改 Guest OS 内核**的虚拟化技术。virtio 虽然是一种半虚拟化 I/O 框架，但它**不需要修改 Guest OS 内核**，只需要安装驱动，因此 virtio 更准确地说是一种**I/O 虚拟化优化技术**，可以在 L-1（全虚拟化）和 L-2（半虚拟化）中使用。本文档将 virtio 归类为 L-2，是因为它通过优化的接口提高性能，符合半虚拟化的性能优化理念，但读者需要理解 virtio 的技术本质是 I/O 虚拟化，而非独立的隔离层次。
...
- **真正的半虚拟化（Paravirtualization）**：需要修改 Guest OS 内核，如 Xen PV
- **I/O 虚拟化优化技术**：不需要修改 Guest OS 内核，只需要安装驱动，如 virtio、Hyper-V Enlightenment
...
> **📌 技术定位说明**：virtio 是一种**半虚拟化 I/O 框架**，但**不是独立的隔离层次**。virtio 的核心是 I/O 虚拟化优化技术，它可以在全虚拟化（L-1）和半虚拟化（L-2）环境中使用，通过优化的接口提高 I/O 性能。virtio **不需要修改 Guest OS 内核**，只需要在 Guest OS 中安装 virtio 驱动（前端驱动），在 Hypervisor 中实现 virtio 后端。
...
| 特性 | 半虚拟化（Xen PV） | virtio |
|------|-------------------|--------|
| **是否需要修改 Guest OS 内核** | ✅ 是（需要修改内核） | ❌ 否（只需要安装驱动） |
| **技术本质** | 虚拟化技术（隔离层次） | I/O 虚拟化优化技术 |
| **适用场景** | 需要修改内核的高性能虚拟化 | 全虚拟化和半虚拟化的 I/O 优化 |
| **兼容性** | 需要特定的 Guest OS 内核 | 兼容标准 Linux 内核 |
