...
- [20. 多集群管理：全面梳理](#20-多集群管理全面梳理)
  ...
  - [20.12 多集群检查清单](#2012-多集群检查清单)
  - [20.13 多集群故障排查](#2013-多集群故障排查)
    - [20.13.1 常见问题](#20131-常见问题)
  - [20.14 参考](#2014-参考)
...
> **💡 隔离层次关联**：K3s 多集群管理涉及 L-3 容器化层（containerd）和 L-4 沙盒
> 化层（WasmEdge），不同集群可以使用不同的运行时。详细的技术解析请参考：
>
...
- [ ] 集群联邦组件已正确安装（Karmada/Clusternet）
- [ ] 控制平面集群正常运行
- [ ] 成员集群已成功注册
- [ ] 集群间网络连通性正常
- [ ] 集群认证和授权配置正确
...
- [ ] 多集群部署策略已配置（PropagationPolicy/PlacementPolicy）
- [ ] 应用可以正常分发到成员集群
- [ ] 应用配置同步正常
- [ ] 应用状态监控正常
- [ ] 应用故障转移机制已测试
...
- **[27. 2025 趋势 - 2025-11-06 最新更新](../27-2025-trends/2025-trends.md#2714-2025-年-11-月-6-日最新更新)** -
  ...
  - **多集群架构**：支持 Wasm 运行时跨集群部署，边缘节点优先使用 WasmEdge

**多集群最佳实践（2025-11-06）**：

- **边缘多集群**：边缘节点使用 K3s + WasmEdge，云端使用 Kubernetes + runc
- **运行时混部**：通过 RuntimeClass 实现跨集群运行时统一管理
- **性能优化**：边缘节点优先使用 WasmEdge（冷启动 <10 ms），提高多集群部署效率
...
- **[17. GitOps 和持续交付](../17-gitops-cicd/gitops-cicd.md)** - GitOps 和
  CI/CD 文档
