...
存储 →HBA→Hypervisor 内核 → 虚拟 SCSI→VM 内核 → 应用**，存在 **两次 I/O 栈穿越**
，延迟增加**50-100μs**。
...
| 内核子系统   | **虚拟化存储机制**                 | **代码路径**                      | **容器化存储机制**           | **代码路径**              | **性能差异根源**            |
| ------------ | ---------------------------------- | --------------------------------- | ---------------------------- | ------------------------- | --------------------------- |
| **块设备层** | `virtio_blk`虚拟块设备             | `request_fn→virtqueue→QEMU→ioctl` | 直接使用`/dev/sdX`或`rbdX`   | `request_fn→driver→DMA`   | 虚拟化**多 3 次上下文切换** |
| **文件系统** | Guest 内独立 ext4 实例             | `ext4→bio→virtio→Host→SCSI`       | 共享 Host ext4，通过绑定挂载 | `ext4→bio→NVMe`           | 虚拟化**双重文件系统开销**  |
| **页缓存**   | Guest Page Cache + Host Page Cache | **双重缓存**                      | 仅有 Host Page Cache         | **单次缓存**              | 虚拟化**内存浪费 30-50%**   |
| **I/O 调度** | Guest CFQ + Host BFQ               | **双重调度延迟**                  | Host BFQ 直接调度            | **单次调度**              | 虚拟化**I/O 延迟增加 50%**  |
| **命名空间** | 无（VM 隔离）                      | **无**                            | Mount Namespace 隔离         | `chroot+绑定挂载`         | 容器化**轻量级隔离**        |
| **cgroups**  | 仅限制 VM 整体 I/O                 | `blkio.weight`                    | 精细到 Pod/容器级            | `blkio.weight+io.latency` | 容器化**QoS 更精细**        |
...
graph TB
    subgraph 物理硬件层
        A[NVMe SSD] --> B[PCIe 4.0 x4]
        A --> C[PCIe 5.0 x4]
        B --> D[DMA引擎]
        D --> E[PRP物理区域页]

        F[FC HBA卡] --> G[SCSI命令队列]
        G --> H[FCP协议]
    ...
    subgraph 内核驱动层
        L[Linux内核5.10+] --> M[NVMe驱动]
        M --> N[块设备层request_queue]
        N --> O[IO调度器: BFQ/MQ-DEADLINE]

        P[iSCSI Initiator] --> Q[TCP/IP协议栈]
        Q --> R[网络块设备]
    ...
    subgraph 文件系统层
        V[ext4文件系统] --> W[inode/dentry缓存]
        V --> X[journal日志]

        Y[XFS文件系统] --> Z[延迟分配]
        Y --> AA[配额管理xfs_quota]

        AB[OverlayFS] --> AC[lowerdir/upperdir/workdir]
        AC --> AD[联合挂载机制]
    end

    subgraph 虚拟化控制层
        AE[VMware ESXi] --> AF[VMFS6]
        AF --> AG[分布式锁ON-DISK locks]
        AG --> AH[VMDK描述文件]
        AH --> AI[PVSCSI控制器]

        AJ[KVM] --> AK[QEMU]
        AK --> AL[virtio-blk/virtio-scsi]
        AL --> AM[Image: QCOW2格式]
        AM --> AN[写时复制backing file]
    end

    subgraph 容器化控制层
        AO[CSI Specification] --> AP[CSI Controller服务]
        AP --> AQ[CreateVolume/DeleteVolume]

        AR[CSI Node服务] --> AS[NodePublishVolume]
        AS --> AT[绑定挂载mount -o bind]

        AU[Kubernetes] --> AV[PersistentVolume]
        AV --> AW[PersistentVolumeClaim]
        AW --> AX[VolumeAttachment]
        AX --> AY[实际挂载状态]
    end

    subgraph 存储后端实现
        AZ[Ceph RBD] --> BA[librbd库]
        BA --> BB[RADOS对象存储]

        BC[Longhorn] --> BD[Replicated Block Device]
        BD --> BE[基于iSCSI协议]

        BF[Portworx] --> BG[KVDB元数据]
        BG --> BH[存储池聚合]

        BI[Local PV] --> BJ[本地磁盘/dev/sdX]
        BJ --> BK[延迟绑定模式]
    end

    subgraph 容器运行时
        BL[containerd] --> BM[Mount命名空间]
        BM --> BN[rootfs准备]
        BN --> BO[overlay2存储驱动]
        BO --> BP[上层可写层+下层镜像层]
    ...
    subgraph 数据保护
        BT[CSI快照] --> BU[VolumeSnapshot CRD]
        BU --> BV[存储后端快照API]

        BW[Velero] --> BX[Restic备份]
        BX --> BY[应用一致性hook]
        BY --> BZ[pre/post备份脚本]
    end

    subgraph 安全隔离
        CA[SELinux] --> CB[svirt MCS标签]
        CB --> CC[卷文件上下文]

        CD[cgroup v2] --> CE[blkio控制器]
        CE --> CF[io.max带宽限制]

        CG[AppArmor] --> CH[容器配置文件]
        CH --> CI[文件访问控制]
    ...
    style AO fill:#bbf,stroke-width:2px
    style AT fill:#fbb,stroke-width:2px
    style BL fill:#bfb,stroke-width:2px
    style BT fill:#bbf,stroke-width:2px
    style CE fill:#f9f,stroke-width:2px
```
...
mindmap
    root((存储架构对比))
        id1[物理基础设施层]
            id11[虚拟化依赖]
                id111[FC HBA卡: QLogic/Emulex]
                id112[iSCSI硬件卸载TOE]
                id113[SAS扩展器]
                id114[RAID控制器BBU]
            id12[容器化依赖]
                id121[NVMe SSD: U.2/M.2]
                id122[SPDK用户态驱动]
                id123[持久化内存PMEM]
                id124[DPU数据处理单元]
            id13[共同演进]
        ...
        id2[内核与驱动层]
            id21[虚拟化内核路径]
                id211[virtio_blk驱动]
                id212[virtio_scsi驱动]
                id213[vhost-scsi内核模块]
                id214[VMM内存拷贝virtqueue]
            id22[容器化内核路径]
                id221[NVMe驱动]
                id222[RBD内核模块]
                id223[OverlayFS内核模块]
            ...
            id23[性能差异]
                id231[虚拟化: 8次上下文切换]
                id232[容器化: 2次切换]
                id233[延迟差距: 50μs vs 5μs]

        id3[文件系统层]
            id31[虚拟化Guest FS]
                id311[ext4独立实例/VM]
                id312[XFS独立日志]
                id313[Guest Page Cache]
                id314[双重缓存问题]
            id32[容器化Host FS]
                id321[ext4/xfs共享]
                id322[OverlayFS联合挂载]
        ...
        id4[控制平面]
            ...
            id42[容器化控制]
                id421[CSI gRPC接口]
                id422[CRD持久化声明]
                id423[StorageClass策略]
                id424[动态分配器external-provisioner]
            id43[配置效率]
                id431[虚拟化: 人工CLI/GUI]
                id432[容器化: 声明式YAML]
                id433[时间差距: 小时 vs 秒]
        ...
        id5[存储后端生态]
            id51[虚拟化后端]
                id511[商业阵列: Dell EMC/HPE]
                id512[vSAN Virtual SAN]
                id513[NFS NAS设备]
            id52[容器化后端]
                id521[Ceph RADOS]
                id522[Longhorn分布式块]
...
graph TD
    ...
    B -->|否| F{性能要求?}
    F -->|极致IOPS<1ms| G{是否本地数据?}
    G -->|是| H[容器化+Local PV+NVMe+XFS]
    G -->|否| I[容器化+Ceph RBD+SSD+replica 3]

    F -->|通用IOPS>5ms| J{团队技能?}
    J -->|K8s专家| K[容器化+Longhorn+3节点]
    J -->|存储专家| L[容器化+Ceph Rook+CephFS]
    J -->|应用开发| M[容器化+公有云CSI(EBS/PD)]
